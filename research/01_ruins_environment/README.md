...
Ruins-Urban-01 is the first implementation module of the
Multi-UAV Cooperative Exploration paper repository. It is a reproducible,
parameterized 3D urban-ruins environment for Ubuntu 20.04, ROS Noetic, Gazebo
Classic, PX4/Prometheus, FUEL, RACER, and MARSIM workflows.

This directory is a ROS package named `ruins_urban_01`. Clone the parent paper
repository into `~/catkin_ws/src`; catkin will discover this package
recursively.
...
|-- experiments/             environment batch protocol and result schema
...
If an old copy such as `ruins_urban_01_old` still contains `package.xml`, move
it outside `~/catkin_ws/src`; otherwise `catkin_make` will report duplicate
package names.
...
```bash
cd ~/catkin_ws/src/multi-uav-cooperative-exploration/research/01_ruins_environment
...
Run the paper repository checks from the repository root:
